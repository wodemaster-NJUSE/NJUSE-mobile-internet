...
//            ChatModel chatModel = ChatModel.builder().userVO(UserMapper.INSTANCE.toUserVO(findByUid(tmpUid))).build();
//            res.add(chatModel);
            ChatModel chatModel = new ChatModel();
            chatModel.setUserVO(UserMapper.INSTANCE.toUserVO(findByUid(tmpUid)));
            res.add(chatModel);
        }
        return res;
    }
    ...
    public List<MomentModel> getZoneMomentModels(String uid){
        ...
        for (String tmp : uidList)
        {
            ...
            {
                MomentEntity momentEntity = momentDao.findByMomentUid(momentUid);
                MomentModel momentModel = MomentMapper.INSTANCE.toMomentModel(momentEntity,user);
                res.add(momentModel);
            }
        }
    ...
    @Override
    public void SetMotto(String uid, String userMotto) {
        userDao.save(userDao
                .findByUid(uid)
                .setMotto(userMotto));
    }
    ...
    @Override
    public void SetGender(String uid, String userGender) {
        userDao.save(userDao
                .findByUid(uid)
                .setUsername(userGender));
    }
